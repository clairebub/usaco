...
public class Dec19_MilkVisits {
    ...
    static boolean is_it_in(int start, int end, int mt) {
        ...
        visited_types.put(start, new HashSet<>());
        visited_types.get(start).add(milk_types[start]);
        ...
        while (!deque.isEmpty()) {
            ...
            for (Integer b : edges.get(a)) {
                if (!visited_types.containsKey(b)) {
                    visited_types.put(b, new HashSet<>());
                    visited_types.get(b).add(milk_types[b]);
                    visited_types.get(b).addAll(visited_types.get(a));
                    deque.add(b);
                }
            }
